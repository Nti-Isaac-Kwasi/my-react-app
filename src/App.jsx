...
// Sanitized App ID to prevent path segment errors (e.g. if ID contains '/')
...
export default function App() {
  ...
  // const [portfolio, setPortfolio] = useState([]); // Reserved for future
      ...
      if (u) {
        ...
        try {
          ...
          await setDoc(userRef, { 
            ...
            // Only set defaults if missing (handled by merge=true nature)
          }, { merge: true });
        } catch (err) {
  ...
  const handleNav = (page) => {
    ...
    setCurrentModule(null); // Close reader if open
  ...
  return (
    <div className="animate-fade-in">
      <h1 className="text-3xl font-display font-bold mb-6 text-gray-900 dark:text-white">
        Hello, {(typeof user.displayName === 'string' ? user.displayName.split(' ')[0] : 'Trader')}! 👋
      </h1>
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="space-y-3">
        {modules.length === 0 && <div className="text-gray-500 italic">No modules loaded yet.</div>}
        {modules.map(m => {
...
function ForexSimulator({ onBack }) {
  ...
  const dataPoints = useRef([1.1050]); // Use ref to avoid re-render loop on canvas draw
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      setPrice(newPrice); // Triggers re-render for UI, maybe throttle this in real app
...
function VideoLibrary({ videos, userData }) {
  // Simple Suggestion Algo: Sort by matching category of user interest
